...
3. If the text is already ${lang} or contains nothing translatable, return it unchanged.`
}
    ...
    async (req, next) => {
      if (req && typeof req.reason === 'string' && req.reason.trim() && /[A-Za-z]{4,}/.test(req.reason)) {
        try {
          ...
          const timer = setTimeout(() => controller.abort(), CALL_TIMEOUT_MS)
          try {
            const out = await translateOne(ctx, api, req.reason, config?.target || 'zh', provider, model, controller.signal)
            if (out && out !== req.reason) req.reason = out
          } finally {
            clearTimeout(timer)
          }
        } catch {
