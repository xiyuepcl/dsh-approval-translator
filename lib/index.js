...
// CJK coverage used by the "already translated" guard.  Chinese is the default
// target and the only locale this guard is currently strict about; other targets
// still fall back to the old Latin-heuristic (translate anything with English).
const CJK_RE = /[\u3400-\u9fff\uF900-\uFAFF]/

// Split a reason like "escalate sandbox to danger-full-access: <explanation>"
// at the first colon that is followed by whitespace.  This keeps Windows drive
// letters ("C:\...") and URLs from being mistaken for a command prefix.
function splitReasonText(text) {
...
function isAlreadyTargetLanguage(text, target) {
  if (!text) return false
  if (target === 'zh' || target === 'zh-CN' || target === 'zh-cn') return CJK_RE.test(text)
  // For other targets the current heuristic stays conservative: only skip when
  // the text is clearly already in that target's script.  Unknown targets are
  // treated as needing translation.
  if (target === 'ja') return /[\u3040-\u30ff]/.test(text)
  if (target === 'ko') return /[\uac00-\ud7af]/.test(text)
  if (target === 'en' || target === 'en-US') {
    return !CJK_RE.test(text) && !/[\u0400-\u04ff\u3040-\u30ff\uac00-\ud7af]/.test(text)
  }
  return false
}
...
export function apply(ctx, config = {}) {
  ...
  ctx.on(
    'approval/request',
    async (req, next) => {
      if (req && typeof req.reason === 'string' && req.reason.trim()) {
        const original = req.reason.trim()
        const target = config?.target || 'zh'


        // Idempotence guard: if another loaded copy (or an earlier pass) has
        // already composed the "translation + original" display, do not run a
        // second translation over the mixed text.
        if (original.includes(ORIGINAL_MARKER)) return next()

        // dsh reasons are often `escalate sandbox to <mode>: <justification>`.
        // The justification is model-authored and may already be Chinese; in
        // that case translating again would produce a redundant Chinese
        // "translation" and an "original" line that is also Chinese.
        const { body } = splitReasonText(original)
        if (!body || isAlreadyTargetLanguage(body, target)) return next()
        if (!/[A-Za-z]{4,}/.test(body)) return next()
        try {
          const api = await llmApi()
          const { provider, model } = await resolveRoute(ctx.llm, config)
          let out = null
